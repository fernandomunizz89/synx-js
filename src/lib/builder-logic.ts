...
export async function loadPreviousBuilderSignature(taskId: string): Promise<string | null> {
...
export function stripPathTips(text: string): string[] {
  const out: string[] = [];
...
export function filterIssuesByScope(values: string[], scopeFiles: string[]): string[] {
  ...
  return uniqueNormalized(values).filter((line) => {
    const paths = stripPathTips(line);
