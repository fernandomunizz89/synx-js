...
describe("builder-logic", () => {
  ...
  describe("stripPathTips", () => {
    it("should extract paths from text", () => {
      const text = "Error in src/app.ts:10:5 and src/utils.js";
      expect(stripPathTips(text)).toEqual(["src/app.ts", "src/utils.js"]);
    });
  });

  describe("filterIssuesByScope", () => {
    it("should filter issues by scope files", () => {
      const issues = ["Error in a.ts", "Error in b.ts", "Generic error"];
      const scope = ["a.ts"];
      const result = filterIssuesByScope(issues, scope);
      expect(result).toEqual(["Error in a.ts", "Generic error"]);
    });

    it("should return all if scope is empty", () => {
      const issues = ["a.ts", "b.ts"];
      expect(filterIssuesByScope(issues, [])).toEqual(["a.ts", "b.ts"]);
    });
  });
