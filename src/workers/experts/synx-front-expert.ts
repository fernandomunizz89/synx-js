 ...
 */
export class SynxFrontExpert extends WorkerBase {
  ...
  protected async processTask(taskId: string, request: StageEnvelope): Promise<void> {
    ...
    const prompt = await loadPromptFile("synx-front-expert.md");
    ...
    if (researchDecision.status === "abort_to_human") {
      ...
      await this.finishStage({
        taskId,
        stage: "synx-front-expert",
        doneFileName: DONE_FILE_NAMES.synxFrontExpert,
        viewFileName: "04-synx-front-expert.md",
...
`;
    ...
    if (!effectiveChanged.length && !gitChangedFiles.length) {
      throw new Error("Synx Front Expert completed but no code changes were detected.");
    }
    ...
    await this.finishStage({
      ...
      nextInputRef: `done/${DONE_FILE_NAMES.synxFrontExpert}`,
      startedAt,
