...
import { SinxSeoSpecialist } from "./sinx-seo-specialist.js";
...
describe.sequential("workers/experts/sinx-seo-specialist", () => {
  let root = "";
  let repoRoot = "";
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(repoRoot, "package.json"), JSON.stringify({ name: "sinx-seo-test" }, null, 2), "utf8");
  ...
  it("processes an SEO task and routes to Sinx QA Engineer", async () => {
    ...
    const inboxPath = path.join(task.taskPath, "inbox", STAGE_FILE_NAMES.sinxSeoSpecialist);
    ...
    const expert = new SinxSeoSpecialist();
    ...
    await writeJson(qaDonePath, {
      ...
      output: {
        ...
        qaHandoffContext: {
          attempt: 1,
          maxRetries: 3,
          returnedTo: "Sinx SEO Specialist",
          summary: "SEO fail",
          latestFindings: [
            {
  ...
  it("exercises conditional logic branches", async () => {
    ...
    const donePath = path.join(task.taskPath, "done", DONE_FILE_NAMES.sinxSeoSpecialist);
